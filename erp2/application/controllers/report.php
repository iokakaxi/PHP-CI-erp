...
class Report extends CI_Controller {
	...
	public function index() {
	    //库存总量 成本
		$where1= ' and f.uid=' . $this->jxcsys['uid'] . '';
		$costini   = $this->data_model->get_invoice_info_ini($where1.' and billDate<="'.date('Y-m-d').'" group by invId');
		
		$inventory1  = $inventory2 = 0;
		$where = ' and a.billDate<="'.date('Y-m-d').'"';
		$where .= ' and f.uid=' . $this->jxcsys['uid'] . '';
	    ...
	    //现金、银行存款
		$fund1 = $fund2 = 0;
		$where2= ' and a.uid=' . $this->jxcsys['uid'] . '';
		$list = $this->data_model->get_account('',$where2);
		foreach ($list as $arr=>$row) {
		    if ($row['type']==1) {
			    $fund1 += $row['amount'];
			} else {
		...
		//客户欠款
		$contact1 = $contact2 = 0;
		$where3= ' and a.uid=' . $this->jxcsys['uid'] . '';
		$where4= ' and uid=' . $this->jxcsys['uid'] . '';
		$list = $this->data_model->get_contact($where4,$where3);
		foreach ($list as $arr=>$row) {
		    if ($row['type']==-10) {
		...
		$where5= ' and f.uid=' . $this->jxcsys['uid'] . '';
		//采购金额
		$list = $this->data_model->get_invoice_info_sum($where5.' and a.transType=150501 and billDate>="'.date('Y-m-1').'" and billDate<="'.date('Y-m-d').'" group by a.invId');
		$purchase1 = 0;
		foreach ($list as $arr=>$row) {
		...
		//销售收入 
		$sales1 = $sales2 = 0;
		$where6= ' and f.uid=' . $this->jxcsys['uid'] . '';
		$list1  = $this->data_model->get_invoice_info_sum($where6.' and billType="SALE" and billDate>="'.date('Y-m-1').'" and billDate<="'.date('Y-m-d').'" group by a.invId, a.locationId');
...
//
	...
	public function salesDetail_invExporter() {
		...
		if ($profit==1) {
			$where1= ' and f.uid=' . $this->jxcsys['uid'] . '';
			$data['info']  = $this->data_model->get_invoice_info_ini($where1.' and billDate<="'.$endDate.'" group by invId');
		}
	...
	public function contactDebt_detail() {
		...
		$where .= ' and a.uid=' . $this->jxcsys['uid'] . '';
		$where1= ' and uid=' . $this->jxcsys['uid'] . '';
		$list = $this->data_model->get_contact($where1,$where. ' order by type desc');
		foreach ($list as $arr=>$row) {
		    $v[$arr]['dbid']        = 0;
			$v[$arr]['debt']        = 0; 
	...
	public function invBalance() {
		...
		$v = array();
		$where2= ' and f.uid=' . $this->jxcsys['uid'] . '';
		$cost   = $this->data_model->get_invoice_info_ini($where2.' and a.billDate<="'.date('Y-m-d').'" group by invId');
	...
	public function invBalance_exporter() {
		...
		$where1 = $storageNo ? ' and locationNo="'.$storageNo.'"' : '';
		$where2= ' and f.uid=' . $this->jxcsys['uid'] . '';
		$data['cost']  = $this->data_model->get_invoice_info_ini($where2.' and billDate<="'.date('Y-m-d').'" group by invId');
		$where1.= ' and uid=' . $this->jxcsys['uid'] . '';
	...
	public function deliverSummary() {
		...
		$data['data']['total']     = ceil($data['data']['records']/$rows);
		$where2= ' and f.uid=' . $this->jxcsys['uid'] . '';
		$info  = $this->data_model->get_invoice_info_ini($where2.' and billDate<="'.$endDate.'" group by invId');
	...
	public function deliverSummary_exporter() {
		...
		$where .= ' and f.uid=' . $this->jxcsys['uid'] . '';
		$where2= ' and f.uid=' . $this->jxcsys['uid'] . '';
		$data['info']   = $this->data_model->get_invoice_info_ini($where2.' and billDate<="'.$endDate.'" group by invId');
...
//		$data['list']   = $this->data_model->get_invoice_info(' and a.billDate<="'.$endDate.'" group by a.invId,a.locationId'); 
	...
	public function bankBalance_detail() {
		...
		$where1 .= ' and d.uid=' . $this->jxcsys['uid'] . '';
	...
	public function customerBalance_detail() {
		...
		$where1 = ' and a.uid=' . $this->jxcsys['uid'] . '';
		$list1 = $this->data_model->get_contact('and billDate<"'.$beginDate.'"'.$where,$where1.' and a.id="'.$customerId.'"',1);
		$where .= $beginDate ? ' and billDate>="'.$beginDate.'"' : '';
		$where .= $endDate ? ' and billDate<="'.$endDate.'"' : '';
		$where2 = ' and uid=' . $this->jxcsys['uid'] . '';
		$list2 = $this->mysql_model->get_results(INVOICE,'(isDelete=0) '.$where.$where2.' order by id');
		$where3 = ' and f.uid=' . $this->jxcsys['uid'] . '';
		$list3 = $this->data_model->get_invoice_info($where.$where3.' order by a.id');
	...
	public function customerBalance_exporter() {
		...
		$where .= ' and buId='.$customerId;
		$where1 = ' and a.uid=' . $this->jxcsys['uid'] . '';
		$data['list1'] = $this->data_model->get_contact('and billDate<"'.$beginDate.'"'.$where,$where1.' and a.id="'.$customerId.'"',1);
		$where .= $beginDate ? ' and billDate>="'.$beginDate.'"' : '';
		$where .= $endDate ? ' and billDate<="'.$endDate.'"' : '';
		$where2 = ' and uid=' . $this->jxcsys['uid'] . '';
		$data['list2'] = $this->mysql_model->get_results(INVOICE,'(isDelete=0) '.$where.$where2.' order by id');
		$where3 = ' and f.uid=' . $this->jxcsys['uid'] . '';
		$data['list3'] = $this->data_model->get_invoice_info($where.$where3.' order by a.id');
	...
	public function supplierBalance_detail() {
		...
		$where1 = ' and a.uid=' . $this->jxcsys['uid'] . '';
		$list1 = $this->data_model->get_contact('and billDate<"'.$beginDate.'"'.$where,$where1.' and a.id="'.$supplierId.'"',1);
		$where .= $beginDate ? ' and billDate>="'.$beginDate.'"' : '';
		$where .= $endDate ? ' and billDate<="'.$endDate.'"' : '';
		$where2 = ' and uid=' . $this->jxcsys['uid'] . '';
		$list2 = $this->mysql_model->get_results(INVOICE,'(isDelete=0) '.$where.$where2.' order by id');
	...
	public function supplierBalance_exporter() {
		...
		$where .= ' and buId='.$supplierId;
		$where1 = ' and a.uid=' . $this->jxcsys['uid'] . '';
		$data['list1'] = $this->data_model->get_contact('and billDate<"'.$beginDate.'"'.$where,$where1.' and a.id="'.$supplierId.'"',1);
		$where .= $beginDate ? ' and billDate>="'.$beginDate.'"' : '';
		$where .= $endDate ? ' and billDate<="'.$endDate.'"' : '';
		$where2 = ' and uid=' . $this->jxcsys['uid'] . '';
		$data['list2'] = $this->mysql_model->get_results(INVOICE,'(isDelete=0) '.$where.$where2.' order by id');
	...
	public function oriDetail_detail() {
		...
		$where .= ' and d.uid=' . $this->jxcsys['uid'] . '';
		$offset = $rows * ($page-1);
		$data['data']['page']      = $page;
