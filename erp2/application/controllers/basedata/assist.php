...
class Assist extends CI_Controller
{
    ...
    public function add()
    {
        ...
        $sql = $this->mysql_model->insert(CATEGORY, elements(array('name', 'typeNumber','uid'), $data));
        if ($sql) {
    ...
    private function trade_update($data)
    {
        if (count($data) > 0) {
            ...
            $this->mysql_model->get_count(CATEGORY, '(id<>' . $id . ') and (uid='. $data['uid'].') and (name="' . $name . '") and typeNumber="' . $data['typeNumber'] . '"') > 0 && str_alert(-1, '类别名称重复');
