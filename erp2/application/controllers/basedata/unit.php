...
class Unit extends CI_Controller {
	...
	public function add(){
		...
		if (count($data)>0) {
			...
			$sql  = $this->mysql_model->insert(UNIT,elements(array('name','default','uid'),$data));
			if ($sql) {
				$data['id'] = $sql;
	...
	private function validform($data) {
		...
		$this->mysql_model->get_count(UNIT,'(isDelete=0) and (name="'.$data['name'].'") and (uid="'.$this->jxcsys['uid'].'")'.$where) && str_alert(-1,'单位名称重复');
		if (isset($data['id'])) {
			$this->mysql_model->get_count(GOODS,'(isDelete=0) and (unitId='.$data['id'].') and (uid="'.$this->jxcsys['uid'].'")')>0 && str_alert(-1,'该单位已经被使用，不允许更改组');
		}
		return $data;
	}  
