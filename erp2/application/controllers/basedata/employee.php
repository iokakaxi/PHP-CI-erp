...
class Employee extends CI_Controller {
	...
	public function index(){
		...
		$list = $this->mysql_model->get_results(STAFF,$where.' and (isDelete=0) order by id desc');
		...
		$data['data']['items']      = $v;
		$data['data']['total']      = $this->mysql_model->get_count(STAFF,$where.' and (isDelete=0)');
		$data['data']['records']    = 3;
	...
	public function add(){
		...
		if (count($data)>0) {
			...
			$sql  = $this->mysql_model->insert(STAFF,elements(array('name','number','uid'),$data));
			if ($sql) {
				$data['id'] = $sql;
