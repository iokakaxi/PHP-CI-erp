...
class Invlocation extends CI_Controller {
	...
	public function index(){
		...
		$list = $this->mysql_model->get_results(STORAGE,$where.' and (isDelete=0) order by id desc');
		...
		$data['data']['rows']       = $v;
		$data['data']['total']      = 1;
		$data['data']['records']    = $this->mysql_model->get_count(STORAGE,$where.' and (isDelete=0)');
		$data['data']['page']       = 1;
	...
	public function add(){
		...
		if (count($data)>0) {
			...
			$sql = $this->mysql_model->insert(STORAGE,elements(array('name','locationNo','uid'),$data));
			if ($sql) {
				$data['id'] = $sql;
