...
class Settacct extends CI_Controller {
	...
	public function index(){
		...
		$list = $this->mysql_model->get_results(ACCOUNT,$where.' and (isDelete=0) order by id');
		...
		$data['data']['items']      = $v;
		$data['data']['totalsize']  = $this->mysql_model->get_count(ACCOUNT,$where.' and (isDelete=0)');
	...
	public function add(){
		...
		if (count($data)>0) {
			...
			$info = elements(array('name','number','amount','date','type','uid'),$data);
