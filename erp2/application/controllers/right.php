...
class Right extends CI_Controller {
	...
	public function isMaxShareUser() {
		die('{"status":200,"data":{"totalUserNum":20,"shareTotal":1},"msg":"success"}');
	}
	...
	public function queryAllUser() {
		...
		$where = '(uid=' . $this->jxcsys['uid'] . ')';
		$where .= ' or (roleid=' . $this->jxcsys['uid'] . ')';
		$list = $this->mysql_model->get_results(ADMIN, $where.' order by roleid');
		...
		$data['data']['items']        = $v;
		$data['data']['shareTotal']   = $this->mysql_model->get_count(ADMIN,$where);
		$data['data']['totalsize']    = $data['data']['shareTotal'];
		$data['data']['corpID']       = 0;
		$data['data']['totalUserNum'] = 20;
