$(function() {
	...
	function c() {
		...
		}),$("#btn-unified").click(function(a) {
			if (a.preventDefault(), Business.verifyRight("INVENTORY_DELETE")) {
				var b = $("#grid").jqGrid("getGridParam", "selarrrow");
				b.length ? h.unified(b.join()) : parent.Public.tips({
					type: 2,
					content: "请选择需要同步的项"
				})
			}
	...
	var d = (parent.SYSTEM, Number(parent.SYSTEM.qtyPlaces), Number(parent.SYSTEM.pricePlaces)),
		...
		h = {
			...
			unified: function(a) {
				$.dialog.confirm("商品将同步电商平台，请确认是否同步？", function() {
					Public.ajaxPost("../basedata/inventory/unified?action=unified", {
						id: a
					}, function(b) {
						if (b && 200 == b.status) {
							var c = b.data.id || [];
		...
		i = {
			...
			statusunified: function(a, b, c) {
				var d = a === !0 ? "已同步" : "未同步",
					e = a === !0 ? "ui-label-success" : "ui-label-default";
