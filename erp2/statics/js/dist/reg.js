...
function(a) {
	function b() {
		a.ajax({
			...
			success: function(a) {
				if (200 == a.status) {
					var b = a.data;
					if (b.shareTotal >= b.totalUserNum) return tankuang(120,"共享用户已经达到上限值：" + b.totalUserNum), !1
				}
			}
		})
	}
	function c() {
		var b = a.trim(a("#userName").val()),
			c = a('<span class="loadings"><i class="ui-incon ui-icon-loading"></i>检查用户名...</span>').insertAfter(a("#userName"));
		a("#registerForm").data("onPost", !0), a.ajax({
			url: "../login/queryUserByName?userName=" + b,
			dataType: "json",
			type: "POST",
			success: function(b) {
	...
	function d() {
		var b = {
			...
			code: a.trim(a("#reg_validatacode").val())
		};
		a("#registerForm").data("onPost", !0), a.ajax({
			url: "../login/addUser",
			data: b,
			...
			dataType: "json",
			success: function(c) {
				a("#registerForm").data("onPost", !1), 200 == c.status ?  tankuang(120, c.msg,suurl): tankuang(120, c.msg);
			},
			error: function() {
				a("#registerForm").data("onPost", !1),tankuang(120,"注册用户失败！请重试")
	...
	a("#registerForm input").on("focus", function() {
		...
		b.preventDefault(), f(a("#registerForm")) && d()
